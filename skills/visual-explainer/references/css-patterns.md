...
### Zoom & Pan Controls (SVG viewBox approach)

Add zoom controls to every `.mermaid-wrap` container. All zoom and pan is done by manipulating the SVG `viewBox` attribute directly — this keeps the diagram **vector-crisp at every zoom level** (no pixelation).
...
**Why SVG viewBox instead of CSS transform or CSS zoom?**

- `transform: scale()` rasterizes the element and scales the bitmap — text and lines get pixelated on zoom.
- CSS `zoom` is better but still rasterizes at certain browser zoom levels, and auto-fit requires manual calculation that doesn't adapt to SVG content.
- **SVG viewBox** tells the browser to re-render vector paths at native resolution for every frame. Text, lines, and shapes stay perfectly sharp at any zoom level. Pan is completely unconstrained — just shift the viewBox origin with no scroll bounds to hit.

**Key principles:**

1. **Auto-fit on load:** After Mermaid renders, JS measures the SVG's natural viewBox, then adjusts it so the diagram fills the container with no scrolling needed.
2. **No `preserveAspectRatio="meet"`:** Use `preserveAspectRatio="none"` and dynamically match the viewBox aspect ratio to the container's aspect ratio in JS. This prevents letterboxing (a small diagram floating inside a large empty container).
3. **Unconstrained pan:** Pan moves the viewBox origin — no scroll bounds, no edges to hit. Users can drag freely in any direction.
4. **Pointer-anchored zoom:** Scroll-to-zoom zooms toward the mouse pointer position, not the container center.
...
.mermaid-wrap {
  ...
  border: 1px solid var(--border);
  border-radius: 12px;
  /* CRITICAL: min-height gives the diagram room. 700px for complex, 400px minimum. */
  min-height: 700px;
  /* overflow:hidden — pan is handled by viewBox, not scrollbars */
  overflow: hidden;
  cursor: grab;
}

/* For shorter diagrams that don't need the full height */
.mermaid-wrap--compact { min-height: 400px; }

/* For very tall vertical flowcharts */
.mermaid-wrap--tall { min-height: 800px; }
...
**CRITICAL:** Set `startOnLoad: false` and call `mermaid.run()` manually so you can hook into the render completion to run auto-fit.

```javascript
mermaid.initialize({
  startOnLoad: false,
  theme: 'base',
  themeVariables: { /* ... your palette ... */ },
  flowchart: {
    useMaxWidth: false, // Let SVG use its natural size; viewBox handles fitting
    htmlLabels: true,
    curve: 'basis'
  }
});
...
Add once at the end of the page. Handles auto-fit, zoom buttons, scroll-to-zoom, and unconstrained drag-to-pan on all `.mermaid-wrap` containers:

```javascript
// ---- SVG viewBox zoom & pan: vector-crisp at every zoom level ----
var diagrams = [];

// Apply viewBox with aspect-ratio correction so SVG fills container completely
function applyViewBox(state) {
  if (!state.svg) return;
  var rect = state.wrap.getBoundingClientRect();
  var containerAR = rect.width / rect.height;
  var vbCenterX = state.vbX + state.vbW / 2;
  var vbCenterY = state.vbY + state.vbH / 2;
  var currentAR = state.vbW / state.vbH;
  var finalW, finalH;
  if (containerAR > currentAR) {
    finalH = state.vbH;
    finalW = finalH * containerAR;
  } else {
    finalW = state.vbW;
    finalH = finalW / containerAR;
  }
  state.svg.setAttribute('viewBox',
    (vbCenterX - finalW / 2) + ' ' + (vbCenterY - finalH / 2) +
    ' ' + finalW + ' ' + finalH
  );
}

// Auto-fit: set viewBox so diagram fills container on load
function autoFitDiagram(state) {
  if (!state.svg) return;
  var mx = state.origVbW * 0.05;
  var my = state.origVbH * 0.05;
  state.vbX = state.origVbX - mx;
  state.vbY = state.origVbY - my;
  state.vbW = state.origVbW + mx * 2;
  state.vbH = state.origVbH + my * 2;
  state.initVbX = state.vbX;
  state.initVbY = state.vbY;
  state.initVbW = state.vbW;
  state.initVbH = state.vbH;
  applyViewBox(state);
}

function getState(wrap) {
  for (var i = 0; i < diagrams.length; i++) {
    if (diagrams[i].wrap === wrap) return diagrams[i];
  }
  return null;
}

// Get the actual rendered viewBox (after AR correction)
function getRenderedVB(state) {
  var rect = state.wrap.getBoundingClientRect();
  var containerAR = rect.width / rect.height;
  var vbCenterX = state.vbX + state.vbW / 2;
  var vbCenterY = state.vbY + state.vbH / 2;
  ...
  var finalW, finalH;
  if (containerAR > currentAR) {
    finalH = state.vbH; finalW = finalH * containerAR;
  } else {
    finalW = state.vbW; finalH = finalW / containerAR;
  }
  return { x: vbCenterX - finalW / 2, y: vbCenterY - finalH / 2, w: finalW, h: finalH };
}

// Convert screen pixel to viewBox coordinate
function screenToVB(state, px, py) {
  var rect = state.wrap.getBoundingClientRect();
  var vb = getRenderedVB(state);
  return {
    x: vb.x + ((px - rect.left) / rect.width) * vb.w,
    y: vb.y + ((py - rect.top) / rect.height) * vb.h
  };
}
  ...
  setTimeout(function() {
    document.querySelectorAll('.mermaid-wrap').forEach(function(wrap) {
      ...
      if (vb && vb.width > 0) {
        ...
        origX = bbox.x; origY = bbox.y; origW = bbox.width; origH = bbox.height;
      }

      // Let viewBox control all sizing
      ...
      var state = {
        wrap: wrap, el: el, svg: svg,
        origVbX: origX, origVbY: origY, origVbW: origW, origVbH: origH,
        vbX: origX, vbY: origY, vbW: origW, vbH: origH,
        initVbX: origX, initVbY: origY, initVbW: origW, initVbH: origH
      };
      diagrams.push(state);
      autoFitDiagram(state);
    });
  }, 150);
});

window.addEventListener('resize', function() { diagrams.forEach(autoFitDiagram); });

// Zoom buttons
function zoomDiagram(btn, factor) {
  var state = getState(btn.closest('.mermaid-wrap'));
  if (!state) return;
  var zf = 1 / factor; // factor>1 = zoom in = shrink viewBox
  var cx = state.vbX + state.vbW / 2;
  var cy = state.vbY + state.vbH / 2;
  state.vbW *= zf; state.vbH *= zf;
  state.vbX = cx - state.vbW / 2;
  state.vbY = cy - state.vbH / 2;
  applyViewBox(state);
}
...
// Per-container event listeners
document.querySelectorAll('.mermaid-wrap').forEach(function(wrap) {
  // Ctrl/Cmd + scroll to zoom toward pointer
  wrap.addEventListener('wheel', function(e) {
    if (!e.ctrlKey && !e.metaKey) return;
    e.preventDefault();
    var state = getState(wrap);
    if (!state) return;
    var zf = e.deltaY > 0 ? 1.12 : 1 / 1.12;
    var pt = screenToVB(state, e.clientX, e.clientY);
    state.vbX = pt.x - (pt.x - state.vbX) * zf;
    state.vbY = pt.y - (pt.y - state.vbY) * zf;
    state.vbW *= zf; state.vbH *= zf;
    applyViewBox(state);
  }, { passive: false });

  // Click-and-drag to pan — completely unconstrained
  var isPanning = false, lastX, lastY;
  ...
  window.addEventListener('mousemove', function(e) {
    ...
    var rvb = getRenderedVB(state);
    state.vbX -= ((e.clientX - lastX) / rect.width) * rvb.w;
    state.vbY -= ((e.clientY - lastY) / rect.height) * rvb.h;
    lastX = e.clientX; lastY = e.clientY;
    applyViewBox(state);
  });
  ...
  // Touch support
  var touchId = null, touchLastX, touchLastY;
  wrap.addEventListener('touchstart', function(e) {
    if (e.touches.length !== 1 || e.target.closest('.zoom-controls')) return;
  ...
  wrap.addEventListener('touchmove', function(e) {
    ...
    var rect = wrap.getBoundingClientRect();
    var rvb = getRenderedVB(state);
    state.vbX -= ((t.clientX - touchLastX) / rect.width) * rvb.w;
    state.vbY -= ((t.clientY - touchLastY) / rect.height) * rvb.h;
...
Scroll-to-zoom requires Ctrl/Cmd+scroll to avoid hijacking normal page scroll. Cursor changes to `grab`/`grabbing` to signal pan mode. Pan is completely unconstrained — no scroll bounds, no edges to hit. Diagrams are vector-crisp at every zoom level because the browser re-renders SVG paths from the viewBox, not from a rasterized bitmap.
