...
**Default template (use first):**
- **Blueprint Technical** (recommended default) — IBM Plex Sans + IBM Plex Mono, deep slate/blue palette (#0d1821), bright cyan accent (#1fb6d6), subtle 20px grid background, excellent readability — see `./references/blueprint-technical-default.md` for complete specification

**Constrained aesthetics (prefer these for variety):**
...
**Usage strategy:** Use Blueprint Technical as the default for most technical documentation, test specs, requirements analysis, and data-heavy pages. Vary to Editorial (for prose-heavy content), Paper/ink (for warm/approachable docs), or IDE-inspired themes (for code-focused content) when generating multiple documents in a session. The swap test: if you replaced your styling with a generic dark theme and nobody would notice the difference, you haven't designed anything.
...
- **For diagrams from specification files:** read `./references/diagram-spec-rendering.md` to learn how to consume JSON diagram specs from loom-requirement-analysis skill
...
**For requirement analysis diagrams:** If passed a diagram specification JSON file (e.g., `req-5399-event-detection-spec.json`), load it and follow the rendering workflow in `./references/diagram-spec-rendering.md` instead of manually creating Mermaid syntax. The spec contains optimized node ordering, layout hints, and complexity handling.
...
**Mermaid containers:** Use `overflow: hidden; position: relative;` on `.mermaid-wrap` containers — never `overflow: auto` (scroll-based panning is bounded and fights the viewBox approach). Add zoom controls (+/−/reset buttons) to every `.mermaid-wrap`. **CRITICAL: Give diagrams generous space.** Set `min-height: 600px` (or 700-800px for complex diagrams with 10+ nodes). Diagrams must never feel cramped or claustrophobic — they need breathing room to be readable.

**Mermaid zoom/pan (SVG viewBox approach):** All zoom and pan must be done by manipulating the SVG `viewBox` attribute — never CSS `transform: scale()` or CSS `zoom` (both rasterize the element and cause pixelation). Key requirements: (1) Initialize Mermaid with `startOnLoad: false` and call `mermaid.run()` so you can hook into the post-render callback to read the SVG's natural dimensions. (2) Set `useMaxWidth: false` in flowchart/graph config so the SVG uses its intrinsic size. (3) Set `preserveAspectRatio="none"` on the SVG and dynamically match the viewBox aspect ratio to the container's aspect ratio in JS — using `"meet"` causes letterboxing (small diagram in a large container). (4) Auto-fit the diagram to fill the container on initial load without scrolling. (5) Pan by shifting the viewBox origin (`minX`, `minY`), not via `scrollLeft`/`scrollTop` — viewBox panning is completely unconstrained in all directions. (6) Zoom toward the pointer position by converting mouse coordinates to SVG-space before and after the zoom step. (7) Pan coordinate conversion must use the *rendered* viewBox dimensions (after AR correction), not raw state values, to get 1:1 mouse-to-diagram movement. See `./references/css-patterns.md` "Zoom & Pan Controls (SVG viewBox approach)" for the full pattern.

**Mermaid scaling:** For 15+ element diagrams, consider the hybrid pattern (simple Mermaid overview + CSS Grid detail cards) instead of one massive graph. The viewBox auto-fit handles most sizing automatically, but very large diagrams benefit from being split. See "Architecture / System Diagrams" below.
...
**Minimize line crossings:** Organize node connections to avoid crossing lines whenever possible. Key strategies: (1) Order nodes left-to-right to match their processing sequence, (2) Group related nodes close together, (3) Use dashed lines (`-.->`) for less-important paths like error/reset conditions to visually distinguish them from main flow, (4) Place fan-in nodes (multiple inputs) and fan-out nodes (multiple outputs) strategically to minimize crossing, (5) Consider using `subgraph` to contain related logic and reduce inter-group crossing. Clean diagrams with minimal crossings are dramatically more readable.
...
**Never use ASCII art for diagrams.** If you're about to create an ASCII flow diagram or data flow visualization, STOP and use Mermaid instead. ASCII art is only acceptable for simple inline examples in code comments, never for primary diagram content in visual explanations.
...
**Typography is the diagram.** Pick a distinctive font pairing from the list in `./references/libraries.md`. Use Blueprint Technical default (IBM Plex Sans + IBM Plex Mono) for the first document, then vary for subsequent documents in the same session.

**Default pairing:**
- **IBM Plex Sans + IBM Plex Mono** (Blueprint Technical default) — reliable, readable, professional

**Forbidden as `--font-body`:** Inter, Roboto, Arial, Helvetica, system-ui alone. These are AI slop signals.

**Good pairings for variety:**
...
- **Bright cyan** (`#1fb6d6`) with deep slate/blue base (`#0d1821`, `#1a2332`) — Blueprint Technical default

**Good accent palettes for variety:**
...
- **Mermaid zoom/pan (vector-crisp)**: Every `.mermaid-wrap` must have zoom controls (+/−/reset), Ctrl/Cmd+scroll zoom-toward-pointer, and click-and-drag panning via SVG `viewBox` manipulation (never CSS transforms). The diagram must auto-fit to fill its container on load. Zooming must stay vector-crisp at every level (no pixelation). Panning must be completely unconstrained — no scroll bounds. Cursor changes to `grab`/`grabbing` during drag. See `./references/css-patterns.md` "Zoom & Pan Controls (SVG viewBox approach)" for the full pattern.
